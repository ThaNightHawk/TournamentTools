    ...
    ws.on("message", (data) => {
        if (HJS(data.toString())) {
            ...
            if (jsonObj.Type === "5") {
                ...
                if (jsonObj.command === "createMatch") {
                    //Generate a random GUID V4
                        ...
                        new Packets.Event({
                            match_created_event: new Packets.Event.MatchCreatedEvent({
                                match: new Models.Match({
                                    guid: guid,
                                    associated_users: [...data.players],
                                    leader: taCR.Self.guid
                                })
                            })
                        })
                    );
                }
                if (jsonObj.command === "updateMatch") {
                    const data = jsonObj.message;

                    const taMatch: Models.Match | undefined = taWS.getMatch(data.guid);
                    if (taMatch) {
                        taCR.loadSong(data.selected_name, data.selected_level, data.selected_difficulty, taMatch as Models.Match);
                    }
                }
                if (jsonObj.command === "deleteMatch") {
                    const data = jsonObj.message;
                ...
                if (jsonObj.command === "startMatch") {
                    const data = jsonObj.message;
                    const taMatch: Models.Match | undefined = taWS.getMatch(data.guid);
                    taCR.playSong(taMatch as Models.Match, data.sync, true, true, false);
                }
                if (jsonObj.command === "returnToMenu") {
                    const data = jsonObj.message;
                    taCR.returnToMenu([...data.players]);
                }
            }
        } else {
...
taWS.on("matchCreated", (m) => {
    ws.send(JSON.stringify({ match_id: m.data.guid }));
...
taWS.on("userAdded", (u) => {
    if (u.data.client_type === 0) {
        ...
        console.log("User added: " + user.name + " (" + user.guid + ")");
...
taWS.on("userUpdated", (u) => {
    if (u.data.client_type === 0) {
        try {
            ...
            console.log("Error in userUpdated: " + error);
        }
    }
});
