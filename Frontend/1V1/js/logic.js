...
function scoreUpdate(player, score, combo, acc, miss, reset) {
	if (P1 == player) {
		P1Acc = acc.toFixed(2);
		...
		if (miss >= 1) {
			...
			document.getElementById("Player1FC").innerHTML = miss + "x";
			if (P1Fc) {
				P1Fc = false;
			}
		}
	...
	if (P2 == player) {
		...
		if (miss >= 1) {
			...
			document.getElementById("Player2FC").innerHTML = miss + "x";
			if (P2Fc) {
				P2Fc = false;
			}
		}
